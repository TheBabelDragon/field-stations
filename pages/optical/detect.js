...
import { sampleRegion } from "./sampler.js";
...
/** Hold the current AUTO candidate unless a new cell is materially better. */
export function holdCandidate(current, incoming, { ratio = 1.35, holdAfterRx = false } = {}) {
  if (holdAfterRx && current) return current;
  if (!incoming) return current;
  if (!current) return incoming;
  if (incoming.i === current.i) {
    return {
      ...incoming,
      nx: current.nx * 0.75 + incoming.nx * 0.25,
      ny: current.ny * 0.75 + incoming.ny * 0.25,
      score: incoming.score,
    };
  }
  if (incoming.score >= current.score * ratio) return incoming;
  return current;
}

export function sliceFrom(samples, t0, symbolMs, nBits) {
  if (!samples.length || symbolMs <= 0) return [];
  let min = 1;
  let max = 0;
